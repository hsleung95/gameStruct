...
int main() {
	...
	while(careerOption != '1' && careerOption != '2' && careerOption !='3'){
		cout << "enter your option by number" << endl;
		cin >> careerOption;
	}
	if(careerOption == '2') myChar = magician(charName, 60,80,5,5,10);
	else if(careerOption == '3') myChar = fighter(charName, 120,50,10,10,5);
	else myChar = adventurer(charName, 80,50,8,8,6);
